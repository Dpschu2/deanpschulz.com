...
function valid() {
  ...
  if ("bio" == document.getElementById("current-p").value.replace(/(<([^>]+)>)/gi, "").toLowerCase()){ document.getElementById('loading-sound').play();
    ...
    var e = new XMLHttpRequest;
      ...
      setTimeout(function() {
      var e;
        (e = document.getElementById("input")).removeAttribute("id"), document.getElementById("typewriter").removeAttribute("id"), (e = document.getElementById("input")).insertAdjacentHTML("afterend", '<div id="input"></div>'), e.removeAttribute("id"), typeWriter(document.getElementById("typewriter").dataset.text, 0)
      }, 1200), setTimeout(function() {
        newInput();
      }, 2e3);
    }, 1100);
